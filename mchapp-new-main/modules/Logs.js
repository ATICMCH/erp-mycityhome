...
Logs = {
    ...
    credentials:{
        // Prefer environment variables. If not present, fallback to local JSON file (for local dev).
        GOOGLE_SERVICE_ACCOUNT_EMAIL : process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || "googledrive@soy-reporter-339408.iam.gserviceaccount.com",
        GOOGLE_PRIVATE_KEY : (function(){
            // If GOOGLE_PRIVATE_KEY is provided as an env var, it may contain literal "\\n" which we normalize to newlines.
            if ( process.env.GOOGLE_PRIVATE_KEY ) return process.env.GOOGLE_PRIVATE_KEY.replace(/\\n/g, '\n')
            // If a file path to the key is provided, try to read it (path stored in env var GOOGLE_PRIVATE_KEY_FILE)
            if ( process.env.GOOGLE_PRIVATE_KEY_FILE ) {
                try { const fs = require('fs'); return fs.readFileSync(process.env.GOOGLE_PRIVATE_KEY_FILE, 'utf8') } catch(e){}
            }
            // Fallback to bundled JSON (only for local dev). Wrap in try/catch to avoid hard crash if file absent.
            try { return require("./credentials/GoogleAPI.json").private_key } catch(e){ return null }
        })()
    },
