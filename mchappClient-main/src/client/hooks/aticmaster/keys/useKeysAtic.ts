...
const useKeysAtic = () => {
    ...
    const [listData, setListData] = useState<Array<IKeys>>([])
    ...
    useEffect(() => {
        ...
        FetchApiServiceInstance.getSingleDataWithFilter(`/api/share/data/totaldata/`, { ...dataFilter }, (err) => {
            ...
            if (statusHttpUS === 200 && data) {
                ...
                FetchApiServiceInstance.getAllWithFilter(`/api/atic/p/keys/`, { ..._dataFilter }, (err) => {
                    const { status } = err.response!
                    statusHttpUS = status
                }).then(data => {
                    if (statusHttpUS === 200 && data) {
                        let _data = (data as Array<IKeys>)
                        setListData([..._data])
                    }
                }).catch(err => {
                    console.log('err: ', err)
    ...
    const handleKeyDown = (e: any) => {
        if (e.key === 'Enter') {
            ...
            setPageCurrent(0)
            FilterInstance.setKey(filterFields)
            setLoading(true)
        }
    }
    ...
    useEffect(() => {
        setFilterFields((current) => ({ ...current, ...FilterInstance.getKey() }))
        setFlagFilter((flagFilter) => { return flagFilter + 1 })
    }, [])
