...
const useGrupoPropietario = () => {
    ...
    useEffect(() => {
        ...
        let dataFilter = {
            ...
            tipo: 'all_grupo_propietarios' // recurden este es el valor que definimos en datatotaldata
        }
        ...
        FetchApiServiceInstance.getSingleDataWithFilter(`/api/share/data/totaldata/`, { ...dataFilter }, (err) => {
            ...
            if ( statusHttpUS === 200 && data ) {
                ...
                FetchApiServiceInstance.getAllWithFilter(`/api/dn/p/grupo/propietario/`, { ..._dataFilter }, (err) => {
                    const { status } = err.response!
                    statusHttpUS = status
                }).then( data => {
                    if ( statusHttpUS === 200 && data ) {
    ...
    useEffect(() => {
        ...
        setFilterFields((current) => ({...current, ...FilterInstance.getPropietario()}))
        setFlagFilter((flagFilter) => { return flagFilter + 1 })
    }, [])
...
export default useGrupoPropietario