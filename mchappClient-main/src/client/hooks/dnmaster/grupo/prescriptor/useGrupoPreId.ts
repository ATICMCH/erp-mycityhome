...
import { ALERT_MSG_CONFIR_DELETE_DATA, DN_GET_TYPE_CATEGORIA_PATH, MSG_ERROR_FIELD, MSG_ERROR_SAVE } from '@/client/helpers/constants'
...
const useGrupoPreId = () => {
    ...
    const handleSave = async() => {
        ...
        if ( flagVR === 0 ) dataDB.flag_vr = 'V' // Visita
        else if ( flagVR === 1 ) dataDB.flag_vr = 'R' // Reserva
        ...
        if ( result ) {
            router.push('/dnmaster/grupo/prescriptores')
        }
    }
    ...
    const handleDelete = async() => {
        ...
        const result = await (
            FetchApiServiceInstance.update(`/api/dn/grupo/${id}/prescriptores/delete`, dataDB, (err) => {
                const { status, data } = err.response!
                // Errores de validación del servidor [API]
                if ( status === 409 ) {
                    let _d = data as { error: string, data: Array<{type: string, code: string, field: string, msg: string, label: string}> }
    ...
    return {
        ...
        handleChange,
        handleSave,
        handleCancel: () => handleCancel('/dnmaster/grupo/prescriptores', router),
        handleGoBackLead,
        errorValidate,
        msgError,
        listSucesos,
