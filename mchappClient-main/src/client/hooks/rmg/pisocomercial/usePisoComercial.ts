...
const usePisoComercial = () => {
    ...
    // AGREGAR nuevo estado para indicar si necesitamos ubicación
    ...
    useEffect(() => {
        ...
        let dataFilter = {  
            ...
            // CAMBIO AQUÍ: detectar automáticamente qué tipo necesitamos
            tipo: needsLocation ? 'pisos_colaborador' : 'all_info_comercial'         
        }
        ...
        FetchApiServiceInstance.getSingleDataWithFilter(`/api/share/data/totaldata/`, { ...dataFilter }, (err) => {
            ...
            if ( statusHttpUS === 200 && data ) {
                ...
                // CAMBIO: Primero intentar colaborador, si falla usar RMG
                const endpoint = `/api/rmg/p/apartments/` // Siempre RMG
                
                FetchApiServiceInstance.getAllWithFilter(endpoint, { ..._dataFilter }, (err) => {
                    const { status } = err.response!
                    statusHttpUS = status
                    
                    // Si colaborador da 403, usar RMG como fallback
                    if (needsLocation && status === 403) {
                        console.log('⚠️ Sin permisos para colaborador, usando RMG')
                        
                        FetchApiServiceInstance.getAllWithFilter(`/api/rmg/p/apartments/`, { ..._dataFilter }, (err2) => {
                            // Procesar con RMG pero avisando que no hay ubicación
                            // ...código normal de RMG...
                        })
                        return
                    }
                }).then( data => {
                    if ( statusHttpUS === 200 && data ) {
                        ...
                        if (needsLocation) {
                            // Formato para vista agrupada (con ciudad)
                            let _dList = _data.map(d => ({
                                ...d,
                                // Agregar campos que RMG espera
                                variablesreserva: d.variablesreserva || [],
                                estado_general: d.estado_general || 1,
                                vr_variablereserva: {},
                                precio_alquiler_tooltip: 0,
                                precio_muebles_tooltip: 0,
                                total_tootip: 0,
                                precio_limite_tooltip: 0,
                                lestado: UtilCustomInstance.getLabelEstadoRMG(d.estado_general || 1),
                                estado_alquiler_rentable: 'NA'
                            }))
                            setListData([..._dList])
                        } else {
                            // MANTENER el procesamiento original de RMG
                            let _dList = [  ..._data.map(d => 
                                    {
    ...
    }, [flagFilter, needsLocation]) // AGREGAR needsLocation como dependencia
    ...
    // Ordenar alfabéticamente por nombre_piso (ajusta el campo si es diferente)
    const listDataSorted = [...listData].sort((a, b) =>
        (a.a_etiqueta || '').localeCompare(b.a_etiqueta || '')
    );

    // En el return, usa listDataSorted en vez de listData
    return {
        listData: listDataSorted,
        loading,
        filterFields,
        total,
        pageCurrent,
        ...
        handlerOnPage,
        handleActionSearch,
        setNeedsLocation // NUEVO: exponer función para cambiar modo
    }
}
