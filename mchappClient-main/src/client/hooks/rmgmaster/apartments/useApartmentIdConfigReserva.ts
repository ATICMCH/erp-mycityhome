    ...
    useEffect(() => {
        ...
        FetchApiServiceInstance.getSingleDataWithFilter(`${RMG_APARTMENT_PATH}${id}/infocomercial`, dataFilter, (err) => {
            ...
            if ( data && statusHttpIPC === 200 ) {
                ...
                setDataDB((el) => ({
                    id: _data.id || 0,
                    nombre_comercial: _data.nombre_comercial || '',
                    link_nombre_comercial: _data.link_nombre_comercial || '',
                    estado_general: _data.estado_general || 0,
                    link_tour_virtual: _data.link_tour_virtual || '',
                    link_calendario_disponibilidad: _data.link_calendario_disponibilidad || '',
                    link_repositorio: _data.link_repositorio || '',
                    tiene_anuncio: _data.tiene_anuncio || false,
                    anuncio_usuario: _data.anuncio_usuario || '',
                    anuncio_contrasenia: _data.anuncio_contrasenia || '',
                    anuncio_plataforma: _data.anuncio_plataforma || '',
                    anuncio_link: _data.anuncio_link || '',
                    idpiso: _data.idpiso || el.idpiso || 0,
                    a_etiqueta: _data.a_etiqueta || '',
                    a_localidad: _data.a_localidad || '',
                    a_codigo_postal: _data.a_codigo_postal || '',
                    a_full_direccion: _data.a_full_direccion ||'',
                    l_idplataforma: 0,
                    l_link: '',
                    plataformas: [],
                    vr_variablereserva: { ...el.vr_variablereserva } as IVariablesReserva,
                    variablesreserva: _data.variablesreserva?_data.variablesreserva.map(el => ({...el, delete(): void {handleDeleteConfigReserva(this.id.toString())}})) : []
                }))
            }
        }).catch(err => {}).finally(()=>{})
