...
import {  MSG_ERROR_FIELD, MSG_ERROR_SAVE, ALERT_MSG_SAVE_DATA } from '@/client/helpers/constants'
...
const useUserId = () => {
    ...
    let id = BigInt((router.query.pid as string) || 0)
    ...
    useEffect(() => {
        ...
        RoleServiceInstance.getAll().then( data => {
            data && setRoles( data.filter(el => el.id.toString() !== 'propietario').map(el => ({ key: el.id.toString(), name: `${el.nombre}`})) )
        }).catch(err => {
            console.log('err: ', err)
        }).finally(()=>{})
    }, [id])
    ...
    return {
        ...
        handleChange,
        handleSave,
        handleCancel: () => handleCancel('/rrhh/users/', router),
        errorValidate,
        msgError,
        dataControl,
        roles
