...
const useEsquema = ( typeTotalData: string, pathGetData: string) => {
    ...
    useEffect(() => {
        ...
        FetchApiServiceInstance.getAllWithFilter(`/api/rrhh/esquema/p/`, { ...dataFilter }, (err) => {
    ...
    const handleKeyDown = (e: any) => {
        if (e.key === 'Enter') {
            ...
            setPageCurrent(0)
            FilterInstance.setEsquema(filterFields)
            setLoading(true)
        }
    }
    ...
    useEffect(() => {
        setFilterFields((current) => ({...current, ...FilterInstance.getEsquema()}))
        setFlagFilter((flagFilter) => { return flagFilter + 1 })
    }, [])
