...
const useKeysAticId = (pathGoToBack: string) => {
    ...
    const handleSave = async() => {
        ...
        const result = await ( id ? FetchApiServiceInstance.update(`/api/atic/keys/${id!}`, dataDB, (err) => {
            ...
            : 
            FetchApiServiceInstance.create('/api/atic/keys', dataDB, (err) => {
                // Se ejecuta para status diferente de 200
                const { status, data } = err.response!
    ...
    useEffect(() => {
        ...
        // Edit
        id && FetchApiServiceInstance.getSingleData(`/api/atic/keys/${id}`, (err) => {
    ...
    const handleDelete = async () => {
        ...
        const result = await ( id ?
            FetchApiServiceInstance.delete(`/api/atic/keys/${id}`, (err) => {
                // Se ejecuta para status diferente de 200
                const { status, data } = err.response!
                // Errores de validación del servidor [API]
                if ( status === 409 ) {
