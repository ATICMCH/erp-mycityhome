...
import { detailsListAlert, detailsNormalAlert, handleCancel, postRequest } from '@/client/helpers/Util'
...
// import { postRequest } from '@/client/helpers/Util';
...
const useLeadId = (pathGoToBack: string) => {
    ...
    const [loadingBtnActions, setLoadingBtnActions] = useState<boolean | undefined>(undefined)
    ...
    const handleSave = async () => {
        ...
        setTiposLeadCustom()
        setLoadingBtnActions(true)

        // Redirecciona si es Avance Sign
        if (flagAvance === 3) {
            setLoadingBtnActions(false)
    ...
    // Post grupo whatsapp
    const handleGrupoWhatsapp = async () => {
        setLoadingBtnActions(true)
        // Guardar telefono [No agregado]
        let dataSend = { ...dataDB }
        let telefono = dataSend.telefono;
        let data = {
            telefono: telefono
        };
        if (dataSend.telefono &&
        ...
        let _result = await (postRequest("/api/dn/whatsappGroup/newGroup/", data, null,(err) => {
                // Se ejecuta para status diferente de 200
                const { status, data } = err.response!
                // Errores de validación del servidor [API]
                if ( status === 409 ) {
