...
const useUserIdSAdmin = (pathGoToBack: string) => {
    ...
    const handleSave = async() => {
        ...
        const result = await ( id ? new UserService().setUser('/api/admin/users', id, dataDB as UserSignup, (err) => {
            ...
            : 
            new UserService().addUser('/api/admin/users', dataDB as UserSignup, (err) => {
                // Se ejecuta para status diferente de 200
                const { status, data } = err.response!
                // Errores de validación del servidor [API]
                if ( status === 409 ) {
    ...
    useEffect(() => {
        ...
        id && new UserService().getById(id, '/api/admin/users/', (err) => {
        ...
        RoleServiceInstance.getAll().then( data => {
            data && setRoles( data.map(el => ({ key: el.id.toString(), name: `${el.nombre}`})) )
        }).catch(err => {
            console.log('err: ', err)
        }).finally(()=>{})
    }, [id])
...
export default useUserIdSAdmin;