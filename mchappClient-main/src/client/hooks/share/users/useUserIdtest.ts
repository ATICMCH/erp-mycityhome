...
const useUserIdd = (pathGoToBack: string) => {
    ...
    const handleDelete = async () => {
        ...
        const result = await (id ?
            FetchApiServiceInstance.delete(`/api/rrhh/users/delete/${id}`, (err) => {
                // Se ejecuta para status diferente de 200
                const { status, data } = err.response!
    ...
    useEffect(() => {
        ...
        id && new UserService().getById(id, '/api/rrhh/users/', (err) => {
            ...
            if (statusHttpSById === 200 && data) {
                //Variables de prueba
                // let _fini = UtilCustomInstance.getDateCurrent().fecha
                // let _fend = ''
                // let _fend2 = ''
                ...
                setDataDB({
                    ...
                    fecha_inicio: UtilCustomInstance.getDateCurrent().fecha,
                    fecha_fin: ''
