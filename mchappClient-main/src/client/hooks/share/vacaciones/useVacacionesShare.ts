...
const useVacacionesShare = ( typeTotalData: string, pathGetData: string) => {
    ...
    useEffect(() => {
        ...
        FetchApiServiceInstance.getAllWithFilter(`/api/share/vacaciones/`, { ...dataFilter }, (err) => {
    ...
    const handleKeyDown = (e: any) => {
        if (e.key === 'Enter') {
            ...
            setPageCurrent(0)
            FilterInstance.setUsersRRHH(filterFields)
            setLoading(true)
        }
    }
    ...
    useEffect(() => {
        setFilterFields((current) => ({...current, ...FilterInstance.getUsersRRHH()}))
        setFlagFilter((flagFilter) => { return flagFilter + 1 })
    }, [])
...
export default useVacacionesShare