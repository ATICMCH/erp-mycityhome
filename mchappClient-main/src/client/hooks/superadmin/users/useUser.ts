import {    ALERT_MSG_CONFIR_DELETE_DATA, 
            DN_APARTMENT_PATH, 
            ALERT_MSG_CONFIR_RESET_PASSWORD,
            ALERT_MSG_RESET_PASSWORD_OK } from '@/client/helpers/constants';
...
const useUser = () => {
    ...
    useEffect(() => {
        ...
        new UserService().getAll('/api/admin/users', (err) => {
            ...
            if ( statusHttpUS === 200 && data ) {
                let _dList = [  ...data.map(d => ( { ...d, 
                        ...
                        actions: [
                            { key: 'edit', label: 'Editar', link: `/superadmin/users/${d.id}`},
                            { key: 'reset', label: 'Resetar contraseña', link: '/superadmin/users', 
                            btnAction: () => {

                                toggleModal()
                                setRowID(d.id/*handleResetPassword(d.id || 0)*/)
                                //handleResetPassword(d.id || 0)
                            }}
                        ] 
                    } ))    
                ]
