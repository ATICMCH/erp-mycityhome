...
const DeviceLockForm = ({ data, handleChange }: { data: IDevice, handleChange: (values: any) => void }) => {
    return (
        <div className='grid grid-cols-3 space-x-2'>
            <div className=" w-full flex text-sm">
                ...
                <input placeholder='MAC' value={data.mac} onChange={handleChange} type="text" name='mac' className="rounded-r-full p-2 w-[85%] outline-blue-800" />
            </div>
            <div className=" w-full flex text-sm">
                ...
                <input placeholder='Codigo Permanente' value={data.codigo_permanente} onChange={handleChange} type="text" name='codigo_permanente' className="rounded-r-full p-2 w-[85%] outline-blue-800" />
            </div>
            <div className=" w-full flex text-sm">
                <label className='px-2 py-2 h-auto w-[2.5rem] bg-[#0077bd] text-white rounded-l-full'>
                    <CiBatteryFull title='Bateria' size={'1.2rem'} />
                </label>
                <input placeholder='Bateria' value={data.bateria} onChange={handleChange} type="number" name='bateria' className="rounded-r-full p-2 w-[85%] outline-blue-800" />
            </div>
        </div>
    )
}
