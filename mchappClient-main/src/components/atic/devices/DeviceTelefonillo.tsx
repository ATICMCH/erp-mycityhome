...
const DeviceTelefonillo = ({ data, handleChange }: { data: IDevice, handleChange: (values: any) => void }) => {
    return (
        <div className='grid grid-cols-3 space-x-2'>
            <div className=" w-full flex text-sm">
                <label className='px-3 py-2 h-auto w-[2.5rem] bg-[#0077bd] text-white rounded-l-full'>
                    <BiMapPin title='Ip Arduino' size={'1.2rem'} />
                </label>
                <input placeholder='Ip Arduino' value={data.ip_arduino} onChange={handleChange} type="text" name='ip_arduino' className="rounded-r-full p-2 w-[85%] outline-blue-800" />
            </div>
        </div>
    )
}

export default DeviceTelefonillo