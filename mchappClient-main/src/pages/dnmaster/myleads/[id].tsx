...
import { AiOutlinePlus, AiFillSave, AiOutlineClose, AiOutlineWhatsApp, AiOutlineMail, AiOutlineComment, AiFillEuroCircle } from 'react-icons/ai'
...
import useMyLeadId from '@/client/hooks/share/myleads/useMyLeadId'
import AreaMMIcon from '@/components/Iconos/AreaMMIcon'
import SpinnerCustom from '@/components/SpinnerCustom'
import { IconType } from 'react-icons/lib'

type floatButtonType = { handler: () => any, Icon: IconType }

const FloatButtons = ({ handler, Icon }: floatButtonType) => (
    <button onClick={handler} className='bg-[#0077bd] text-white p-3 text-lg border border-blue rounded-full hover:bg-white hover:text-blue-800 duration-300' type='button'><Icon size={'2rem'} /></button>
)

const MyLeadId = () => {
    const _pathGoToBack = '/dnmaster/myleads'
    ...
    const { dataDB,
        ...
        setFlagAvance,
        loadingBtnActions,
        loadingBtnActionDelete } = useMyLeadId(_pathGoToBack)
                    ...
                    <div className="bg-[#ffffff72] h-full w-full rounded-2xl shadow-2xl p-5">
                        ...
                        <div className="min-h-[35rem] grid grid-cols-2 space-x-5">
                                ...
                                <div className="bg-[#5da7d5c0] rounded-2xl p-5 space-y-2 h-full">
                                    ...
                                    { dataDB.tipo_lead === 'Propietario' ? 
                                        <div className='grid grid-cols-2 space-x-2'>
                                            <div className="w-full flex text-xs">
                                                <label className='px-2 py-2 h-auto w-[2.5rem] bg-[#0077bd] text-white rounded-l-full col-span-2'>
                                                    <AiFillEuroCircle title='Precio' size={'1.2rem'} />
                                                </label>
                                                <input placeholder='Precio' ref={refTelefono} value={dataDB.precio} onChange={handleChange} type='number' name='precio' className="p-2 w-[100%] rounded-r-full outline-blue-800" />
                                            </div>
                                            <div className="w-full flex text-xs">
                                                <label className='px-2 py-2 h-auto w-[2.5rem] bg-[#0077bd] text-white rounded-l-full col-span-2'>
                                                    <AreaMMIcon title="Área m2" color={'white'} className='w-[1.2rem] h-[1.2rem]' />
                                                </label>
                                                <input placeholder='Área m2' ref={refCorreo} value={dataDB.m2} onChange={handleChange} type='number' name='m2' className="p-2 w-[100%] rounded-r-full outline-blue-800" />
                                            </div>
                                        </div> 
                                        : ''
                                    }
                            ...
                            <div className="bg-[#ffffff5f] rounded-2xl p-5 space-y-2 h-full">
                                <h1 className='text-lg text-[#0077bd] font-bold'>Historial:</h1><p style={{fontSize:"10px"}}>buenas, te llamo de MyCity, nos tienes situados?... El club de propietarios?<br/><br/>

Nosotros, como tu, SOMOS PROPIETARIOS, Hemos montado una agrupación para ayudarnos, somos ya más de 500. <br/>
Creo que te puede interesar, me encantaría contarte o igual te mando un wpp si te pillo en mal momento y te llamo más adelante. 
<br/><br/>
Como te decía, todos los propietarios participan<br/>
MyCityHome.es granito de arena está en ocuparnos del día a día del piso. 

Se rompe algo lo arreglamos; Se va un inquilino localizamos a otro. No dan mucho trabajo, por eso te lo podemos hacer gratuitamente, pero puede llegar a ser muy tedioso	
<br/><br/>
Al final nos encontramos con<br/>
●	Inquilino más contento	por los servicios que le ofrecemos <br/>
●	Propietario más tranquilo 	por que filtramos los problemas<br/>
●	MCH encantado por poder vender servicios adicionales.<br/>
<br/>
Insistir un poco en eso, el 100% de la renta va al propietario. <br/>

Contar la historia.</p>
                                {/* <div className='grid grid-cols-2 space-x-2'>
                                    <div className=" w-full flex text-xs">
                                        <label className='p-2 h-min w-[8rem] bg-[#0077bd] text-white rounded-l-full col-span-2'>F. ult. cambio</label>
                                ...
                                <div className="w-full flex text-sm">
                                    ...
                                    <textarea placeholder='Ingresar comentarios' defaultValue={dataDB.comentario} onChange={handleChange} onInput={e => { const target = e.target as HTMLTextAreaElement; target.style.height = 'auto'; target.style.height = target.scrollHeight + 'px'; }} className="p-3 w-[100%] outline-blue-800 border-none resize-none" name="comentario" style={{ overflow: 'hidden', borderRadius: '0px' }}></textarea>                                </div>
                                {/* Solo para edición*/}
                ...
                <ButtonContainerVertical>
                    <FloatButton disabledEvent={true} stateDisabled={loadingBtnActions} title='Guardar' handler={handleSave} Icon={AiFillSave} />
                    <FloatButton title='Contratar' handler={handleContratarModal} Icon={FaHandshake} />
                    <FloatButton disabledEvent={true} stateDisabled={loadingBtnActionDelete} title='Eliminar' handler={handleDelete} Icon={FaTrashAlt} />
            ...
            <div className='btnContent w-full flex justify-end sticky'>
                {
                    ( validateCodeGroup() ) ?
                    <button onClick={handleContratar} disabled={ loadingScreen === undefined ? false : loadingScreen } className='w-[5rem] p-2 hover:text-white text-blue-700 border border-[#0077bd] rounded-full hover:bg-[#0077bd]' type='button'>
                        {( loadingScreen === undefined ) ? 'Listo' : (loadingScreen) ? <SpinnerCustom msn='' /> : 'Listo'}
                    </button>
                    :
                    ''
                }
