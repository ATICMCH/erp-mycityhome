...
import useLeadId from "@/client/hooks/dnmaster/leads/useLeadId";
...
const LeadId = () => {
  ...
  return (
    <Layout>
                ...
                <div className="bg-[#5da7d5c0] rounded-2xl p-5 space-y-2 h-full">
                  ...
                  <div className="w-min grid space-x-2">
                    <div className="flex text-sm">
                      <EnumButtons
                        txtSize="sm"
                        values={[...listInteresa.map((el) => el.name)]}
                  ...
                  {dataDB.tipo_lead === "Propietario" ? (
                    <div className="grid space-x-2">
                      <div className="w-auto flex text-sm">
                        <EnumButtons
                          txtSize="sm"
                          values={[...listOcupacion.map((el) => el.name)]}
                  ...
                  <div className="w-min grid space-x-2">
                    <div className="flex text-sm">
                      <EnumButtons
                        txtSize="sm"
                        values={[...listAvance.map((el) => el.name)]}
                        selected={flagAvance}
                    ...
                    : dataDB.historico.map((el, index) => (
                        ...
                        >
                          ...
                          <div className="text-xs w-full text-blue-800 font-semibold text-right">
                            {`${el.responsable}, `}{" "}
                            <span className="">{`${el.fecha_creacion_short}`}</span>
                          </div>
                        </div>
                      ))}
                </div>
      ...
      <Modal
        title={`Contratar ${
          dataDB.tipo_lead === "Propietario" ? "Propietario" : "Prescriptor"
        }`}
        isOpen={isModalOpen}
      >
        <>
            ...
            <div className="w-full grid max-h-[15rem] overflow-y-scroll">
              {listGrupo.map((el, index) => (
                <div key={`grp-${index}`} className="w-full flex text-sm">
                  <div
                    onClick={() => handleChangeGrupo(parseInt(el.key))}
                    className="flex items-center mr-4 py-[0.4rem] cursor-pointer"
                  >
                    <input
                      checked={dataDB.grupo?.id.toString() === el.key}
                      onChange={(e) => {}}
                      type="radio"
                      value={`${el.key}`}
                      name="grupo"
                    ...
                    <>
                      <div className="w-full space-x-2">
                        <div className="w-[100%] flex text-black text-xs">
                          ...
                          />
                          <div
                            onClick={() => handleActionGrupo("delete")}
